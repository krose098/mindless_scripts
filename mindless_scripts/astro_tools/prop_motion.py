...
def prop_motion(coords, pm_ra, pm_dec, old_time, new_time, distance=None):
    """
...
def main(coords, pm_ra, pm_dec, old_time, new_time, distance, parallax,galactic):
    pos_eq, __ = unicoord(coords,galactic,display=False)
    if parallax is True:
        distance=Distance(parallax = distance*u.mas)
    else:
        distance=Distance(distance*u.pc)
    prop_motion(pos_eq,pm_ra, pm_dec, old_time, new_time, distance )
    return
